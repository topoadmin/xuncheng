...
(function(factory) {
	...
	address.prototype._ready = function() {
		...
		html.push('<div class="am-u-sm-4 am-address-prov"><ul class="am-list am-list-static"></ul></div>');
		html.push('<div class="am-u-sm-4 am-address-city"><ul class="am-list am-list-static"></ul></div>');
		html.push('<div class="am-u-sm-4 am-address-district"><ul class="am-list am-list-static"></ul></div>');
		html.push('</div>');
	...
	address.prototype._readyProv = function(methods) {
		...
		if(methods == "create") {
			...
			self.$prov.find("ul").html(prov.list).data("address-active", prov.index);
			if(!prov.index) {
	...
	address.prototype._readyCity = function(methods, onEvent) {
		...
		if(methods == "create") {
			...
			if(!cityJson) {
				self.$city.find("ul").html("");
				self.$district.find("ul").html("");
				if(onEvent == "tap") {
					self._selectEnd();
				}
				return false;
			...
			var city = address._readyList(cityJson, self.options.city);
			self.$city.find("ul").html(city.list).data("address-active", city.index);
			...
			if(onEvent === "tap" && cityJson.length <= 1 && self.options.selectNumber == 2) {
				self._selectEnd();
			}
	...
	address.prototype._readyDistrict = function(methods, onEvent) {
		...
		if(methods == "create") {
			...
			if(!districtJson) { // 如果县级数据不存在，设置为空
				self.$district.find("ul").html("");
				if(onEvent == "tap") {
					self._selectEnd();
				}
			...
			self.$district.find("ul").html(district.list).data("address-active", district.index);
			if(!district.index) {
	...
	address.prototype._selectEnd = function() {
		...
		if(self.options.selectNumber != 1) { // selectNumber = 1 表示只选省级 
			...
			var cityJson;
			if(provJson.city) {
				cityJson = provJson.city[cityIndex];
			}
			if(cityJson) {
				sJson.city = cityJson.name;
				sJson.zip = cityJson.zip;
				// 判断是否有 县区 级数据
				if(cityJson.district && cityJson.district.length && self.options.selectNumber != 2) { // selectNumber = 2 表示只选省市级
